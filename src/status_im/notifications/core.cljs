...
(when-not platform/desktop?
  ...
  (fx/defn handle-on-message
    ...
    (let [view-id            (:view-id db)
      ...
      (when-not (and (= app-state "active")
        ...
        (when (and (valid-notification-payload? rehydrated-payload)
                   (and (accounts.db/logged-in? cofx)
  ...
  (fx/defn handle-push-notification-open
    ...
    (let [current-public-key (accounts.db/current-public-key cofx)
      ...
      (log/debug "handle-push-notification-open" "current-public-key:" current-public-key "rehydrated-payload:" rehydrated-payload "stored?:" stored?)
